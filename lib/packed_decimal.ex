defmodule PackedDecimal do
  ...
  * coefficient * 10 ^ exponent`. All numbers are represented and calculated
  ...
  """
  @spec nan?(t) :: boolean
  def nan?(packed_decimal()) when nan == 1, do: (silence_unused(); true)
  ...
  """
  @spec inf?(t) :: boolean
  def inf?(packed_decimal()) when inf == 1, do: (silence_unused(); true)
  ...
  """
  @spec sign(t) :: sign()
  def sign(packed_decimal()) when sign == 1, do: (silence_unused(); 1)
  def sign(packed_decimal()) when sign == 0, do: (silence_unused(); -1)
  ...
  """
  @spec abs(t) :: t
  def abs(packed_decimal() = packed_decimal) when sign == 1 do
    silence_unused()
    packed_decimal
  end
...
#
  ...
  def parse("-" <> rest) do
    case parse_unsign(rest) do
      {%PackedDecimal{} = num, rest} -> {put_sign(num, 0), rest}
      :error -> :error
    end
  end
  ...
  def to_string(packed_decimal(), _type) when inf == 1 do
    silence_unused()
  ...
  def to_integer(packed_decimal()) when exp == 0 and nan == 0 and inf == 0 do
    sign(sign) * coef
  end
  ...
  def to_float(packed_decimal()) when nan == 0 and inf == 0 do
  ...
  def integer?(packed_decimal()) when nan == 1, do: (silence_unused(); false)
  def integer?(packed_decimal()) when inf == 1, do: (silence_unused(); false)
  def integer?(packed_decimal()), do: (silence_unused(); exp >= 0 or zero_after_dot?(coef, exp))
  ...
  defp precision(packed_decimal() = num, _precision, _rounding) when nan == 1 do
    silence_unused()
    {num, []}
  end

  defp precision(packed_decimal() = num, _precision, _rounding) when inf == 1 do
    silence_unused()
    {num, []}
  end
...
#   ## CONTEXT ##
  ...
  defp decimal(%PackedDecimal{} = num), do: num
